...
# --- ส่วนเพิ่มใหม่: ระบบสำรองและกู้คืนข้อมูล (Backup & Restore) ---
st.subheader("🛡️ ระบบสำรองและกู้คืนข้อมูล (Backup & Restore)")
st.markdown("คุณสามารถดาวน์โหลดข้อมูลบัญชีทั้งหมดเก็บไว้เป็นไฟล์ CSV หรืออัปโหลดไฟล์ CSV เก่าเพื่อกู้คืนข้อมูลกลับเข้า Google Sheets ได้ที่นี่ครับ")
...
with col_bk1:
    ...
    if not df.empty:
        ...
        st.info("ยังไม่มีข้อมูลสำหรับดาวน์โหลดสำรอง")
...
with col_bk2:
    ...
    if uploaded_file is not None:
        try:
            ...
            if missing_cols:
                st.error(f"ไฟล์ CSV ขาดคอลัมน์สำคัญ: {missing_cols}")
            else:
                st.write("ตัวอย่างข้อมูลที่จะกู้คืน:", restore_df.head(3))
                if st.button("⚠️ ยืนยันการกู้คืนข้อมูลทับทั้งหมด"):
                    for col in ["รายรับ", "รายจ่าย"]:
                        restore_df[col] = pd.to_numeric(restore_df[col], errors='coerce').fillna(0)
                    save_data(restore_df[columns_order])
                    st.success("กู้คืนข้อมูลและบันทึกทับลง Google Sheets สำเร็จเรียบร้อยครับ!")
                    st.rerun()
        except Exception as e:
