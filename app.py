...
st.markdown("""
    ...
    h1 {
        ...
        color: #db2777 !important; /* ชมพูเข้มหรูหรา */
    ...
    /* แก้ไขช่องกรอกข้อมูลให้มีกรอบสี่เหลี่ยมล้อมรอบชัดเจนทุกด้าน ไม่ใช่แค่ซ้ายขวา */
    section[data-testid="stSidebar"] input, 
    section[data-testid="stSidebar"] select,
    section[data-testid="stSidebar"] div[data-baseweb="select"] > div,
    div[data-testid="stSidebar"] div[data-baseweb="base-input"] {
        background-color: #fff1f2 !important;
        border: 2.5px solid #f43f5e !important; /* กรอบสีชมพูแดงเข้มชัดเจน */
    ...
    /* ตอนคลิกช่องกรอกข้อมูล เปลี่ยนเป็นสีทองเรืองแสง */
    ...
    /* ปุ่มบันทึกข้อมูล (สีชมพูบานเย็น-แดงอมชมพู เด้งนูนเด่นชัด) */
    div.stButton > button {
    ...
    /* ปุ่มอัปเดตข้อมูล (สีส้มทองพรีเมียม) */
    ...
    /* ปุ่มพิมพ์รายงาน Excel (สีชมพูอมม่วงสดใส) */
    ...
    /* ตาราง Data Editor ให้ขอบมนสวยงาม */
...
all_accounts = ["ทั้งหมด"] + list(df['บัญชี'].unique()) if not df.empty else ["ทั้งหมด"]
...
if not df.empty:
    ...
    summary = pd.DataFrame(columns=["บัญชี", "รายรับ", "รายจ่าย", "ยอดคงเหลือ"])
...
if not summary.empty:
    ...
    st.info("💡 ยังไม่มีข้อมูลบัญชี กรุณาเพิ่มรายการทางด้านซ้ายครับ")
...
if not summary.empty:
    st.divider()
    fig = px.bar(summary, x='บัญชี', y='ยอดคงเหลือ', color='บัญชี', title="📊 กราฟเปรียบเทียบยอดคงเหลือแต่ละบัญชี")
